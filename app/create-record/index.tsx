...
export default function CreateRecordScreen() {
  ...
  const canSubmit = title.trim() && date.trim() && !submitting;
  ...
  const handleSubmit = async () => {
    ...
    try {
      await recordsApi.create(currentTeamId, {
        type, title, date, location: location || "",
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: Colors.gray[0] }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ gap: 16 }}>
          <Field label="제목 *" value={title} onChange={setTitle} placeholder="예: vs FC 썬더" />
          <Field label="날짜 *" value={date} onChange={setDate} placeholder="2026-04-12" />
          <Field label="장소" value={location} onChange={setLocation} placeholder="강남 풋살장" />
          {type === "match" && (
            <>
              <Field label="상대팀" value={opponent} onChange={setOpponent} placeholder="상대 팀 이름" />
              <View>
